// src/components/admin/leavePolicy/LeavePolicyManagement.tsx
...
/**
 * LeavePolicyManagement - Admin UI (matches HolidayManagement look & behavior)
 * - Header with eyebrow & actions (no breadcrumb)
 * - Centered card + table with centered headers
 * - Search, Export, Print, New, Refresh
 * - Create/Edit modal with focus handling and saving state
 */
...
const LeavePolicyManagement: React.FC = () => {
    ...
    // header actions (placed into Header.actions)
    const headerActions = (
        <>
            <div className="input-group input-group-sm leave-header-search" style={{ minWidth: 280 }}>
                <input
                    value={q}
                ...
                />
                <button className="btn btn-outline-secondary" onClick={() => setQ("")} title="Clear search" aria-label="Clear search">
                    ✕
                </button>
            </div>
            ...
            <button className="btn btn-primary btn-sm" onClick={openCreate} aria-label="Create new policy">
                <i className="bi bi-plus-lg me-1" /> New Policy
            </button>

            <button className="btn btn-outline-secondary btn-sm" onClick={load} aria-label="Refresh policies">
                {loading ? (
                    <>
