...
const UserManagement: React.FC = () => {
  ...
  useEffect(() => {
    fetchUsers();
    // optional: eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  return (
    <div className="user-management container-fluid py-3">
      <h1 className="um-title">User Management</h1>
      <p className="lead">Welcome, {user?.name}</p>

      <div className="um-stats row g-3 mb-4">
      ...
      <div className="card mb-4 p-3 shadow-sm">
        <div className="d-flex flex-column flex-md-row gap-2 align-items-center justify-content-between">
          <div className="d-flex gap-2 w-100">
            ...
            />
            <select className="form-select w-auto" value={roleFilter} onChange={(e) => handleRoleSelect(e.target.value)}>
              <option value="ALL">All</option>
              <option value="USER">User</option>
              <option value="ADMIN">Admin</option>
            </select>
          </div>
          <button className="btn btn-outline-secondary mt-2 mt-md-0" onClick={fetchUsers} disabled={loading}>
            Refresh
          </button>
        </div>
      </div>
              ...
              <tbody>
                {filtered.map((u) => (
                  <tr key={u.userId}>
                    ...
                    <td>{u.role}</td>
                    <td>
                      <span className={`badge ${u.isActive ? "bg-success" : "bg-secondary"}`}>{u.isActive ? "Active" : "Inactive"}</span>
                    </td>
                    <td>
                      <span className={`badge ${u.isAccountLocked ? "bg-danger" : "bg-success"}`}>{u.isAccountLocked ? "Locked" : "Unlocked"}</span>
                    </td>
